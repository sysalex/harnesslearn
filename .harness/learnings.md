...
## 2026-04-24 - 中文文件读写和终端显示要分开判断

### 背景

多模块结构迁移后，`attendance-server-*` 下部分 Java 注释出现真实中文乱码，同时 PowerShell 默认 `Get-Content` 读取 UTF-8 文档时也会显示成乱码，容易混淆“文件已损坏”和“终端显示错误”。

### 学到的内容

- 判断中文是否真的损坏时，先用 `Get-Content -Encoding UTF8` 或搜索典型 mojibake 字符确认，不要只看 PowerShell 默认输出。
- 如果乱码已经写入源码，优先从 Git 历史或原始语义恢复注释，不要继续用错误编码链路批量重写。
- 修复后用 `rg` 扫典型乱码特征，再跑一次 Maven 测试，能同时覆盖文本和编译层面的回归风险。

### 后续建议

- 后续在 Windows 下读写包含中文的源码和文档时，命令里显式指定 UTF-8。
- 结构迁移或批量重写后，把“乱码特征扫描”作为轻量自检步骤加入收尾检查。
